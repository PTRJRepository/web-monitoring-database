...
-- Periksa apakah tabel Gwscannerdata dan PR_TASKREG ada
IF OBJECT_ID('staging_PTRJ_iFES_Plantware.dbo.Gwscannerdata', 'U') IS NOT NULL 
   AND OBJECT_ID('db_ptrj.dbo.PR_TASKREG', 'U') IS NOT NULL
BEGIN
...
BEGIN
    ...
    SELECT 
        ...
        NULL AS TRANSSTATUS,
        NULL AS ISCONTRACT,
        NULL AS ItechUpdateStatus,
        NULL AS DATECREATED,
        NULL AS INTEGRATETIME,
        NULL AS SCANOUTDATETIME,
        NULL AS REVIEWSTATUS,
        NULL AS StatusDiTaskReg,
        NULL AS JumlahTransaksi
    WHERE 
